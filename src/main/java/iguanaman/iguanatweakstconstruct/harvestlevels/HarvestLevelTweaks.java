...
public final class HarvestLevelTweaks {
    ...
    private static void modifyVanillaBlocks() {
        ...
        Blocks.diamond_ore.setHarvestLevel(
                "pickaxe",
                HarvestLevels._4_bronze); // yes, diamond requires diamond level. good thing there's bronze/steel ;)
            ...
            try {
                if (Config.logHarvestLevelChanges) {
                    Log.debug(String.format(
                            ...
                            stack.getUnlocalizedName(), block.getHarvestLevel(m), harvestLevel));
                }
                ...
                if (Config.logOverrideChanges && Loader.instance().isInState(LoaderState.POSTINITIALIZATION))
                    Log.info(String.format(
                            ...
                            stack.getUnlocalizedName(), harvestLevel));
            } catch (Exception e) {
            ...
            for (String toolClass : item.getToolClasses(tmp)) {
                ...
                if (Config.logMiningLevelChanges)
                    Log.debug(String.format(
                            ...
                            toolClass, item.getUnlocalizedName(), old, hlvl));
            }
        }
    ...
    public static void updateToolHarvestLevel(Item item, String toolClass, int hlvl) {
        ...
        if (item.getHarvestLevel(new ItemStack(item), toolClass) != hlvl)
            Log.error("Could not set harvestlevel of " + item.getUnlocalizedName()
    ...
    static {
        String[][][] lists = new String[][][] {
            oreDictLevels, oreDictLevelsMetallurgyFantasy, oreDictLevelsMetallurgyNether, oreDictLevelsMetallurgyEnd
        };
