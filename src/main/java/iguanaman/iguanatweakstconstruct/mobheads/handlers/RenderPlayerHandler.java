...
public class RenderPlayerHandler {
    ...
    public void renderSkullHelmet(RenderPlayerEvent.SetArmorModel event) {
        ...
        boolean isBucket = IguanaTweaksTConstruct.pulsar.isPulseLoaded(Reference.PULSE_ITEMS)
                && item == IguanaMobHeads.wearables;
        if (item != IguanaMobHeads.skullItem && !isBucket) return;

        GL11.glPushMatrix();
        event.renderer.modelBipedMain.bipedHead.postRender(0.0625F);
        float f1 = 1.0625F;
        GL11.glScalef(f1, -f1, -f1);
        if (isBucket) IguanaTileEntitySkullRenderer.renderer
                .renderBucket(-0.5f, 0.0f, -0.5f, 180.0f, 1, itemStack.getItemDamage());
        else IguanaTileEntitySkullRenderer.renderer
                .renderSkull(-0.5f, 0.0f, -0.5f, 180.0f, 1, itemStack.getItemDamage());
